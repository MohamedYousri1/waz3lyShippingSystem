...
export class ExpenseComponent implements OnInit {
  ...
  selectedNode: any ;
  constructor(
    private  _treasury  : ExpenseService ,
  ...
  {
    debugger
    this._treasury.get_allExpense().subscribe((res : any  ) =>  {
      this.files1 = <TreeNode[]> res.expense;
...
editTreasury()
{
  ...
  {
    this._treasury.edit_expense({label : this.treasuryName} , this.nodeId).subscribe(res => {
      this.getIncomes() ;
...
get_mainIncomes()
{
  this._treasury.get_mainExpense().subscribe((res : any ) =>  {
    this.allMainExpense  = res.expense   ;
  },err=> {
    this.allMainExpense = [] ;
  })
}
...
addTreasury()
{
  ...
  }else if (this.addingTreasuryForm.controls.expense_id.value  == null &&  this.addingTreasuryForm.controls.label.value  != null )
  {
    this.addingTreasuryForm.removeControl('expense_id') ;
    this.addingTreasuryForm.addControl('expense_id' , this.fb.control(null)) ;
    this._treasury.adding_expense(this.addingTreasuryForm.value).subscribe(res =>  {
    this.x.add({key: 'myKey2', severity:'success', summary: 'Notification', detail: 'Data Saved Successfully'});
      this.getIncomes() ;
      this.modal.dismissAll() ;
      this.get_mainIncomes()
