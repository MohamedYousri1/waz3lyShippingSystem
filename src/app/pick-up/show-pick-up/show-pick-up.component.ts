  ...
  this.confirmationService.confirm({
    ...
    accept: () => {
      this._pickServ.delete_section(item).subscribe( (data) =>{
        this.x.add({key: 'Key2', severity:'success', summary: 'Notification', detail: ' تم حذف الطلب'});
        this.getPickups();
      },err =>{
        this.x.add({key: 'Key1', severity:'info', summary: 'Notification', detail: '  لا يمكن حذف الطلب  '});
        console.log(err)
      })
    }
});
